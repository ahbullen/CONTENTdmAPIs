...
function getStreamingObjects($collection, $tableStr, $pageFile, $pagePtr) {
	$tableStr = $tableStr . "<tr>\n<td width=\"100%\">\n";
	$typeOfFile = explode ('.',$pageFile);
	if ($typeOfFile[1] != "jp2") {
		$urlStr = "https://server16614.contentdm.oclc.org/dmwebservices/index.php?q=dmGetStreamingFile/$collection/$pageFile/xml";
	}
	else {
	...
	$tableStr = "<table width=\"80%\" border=\"1\">\n";
        ...
        if ($cpd == "Monograph") {
	        foreach ($coXML->xpath('//node') as $coRecord) {
					...
					$tableStr = getStreamingObjects($collection, $tableStr, $pageFile, $pagePtr);
			}
                }
	}
        else {
